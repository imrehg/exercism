...
def __exchange_with_spread(budget: float, exchange_rate: float, spread: int) -> float:
    """Do an exchange conversion taking a spread into account
...
def non_exchangeable_value(budget: float, exchange_rate: float, spread: int, denomination: int) -> int:
    ...
    """
    return int(
        __exchange_with_spread(budget, exchange_rate, spread)
